...
let dbinfo = require('./config')

//1.mysql  연결정보기반으로 객체 생성  
let pool = mysql.createPool({
    connectionLimit:10,    //연결이 안될경우 10초간 기다려봄 10초 넘으면 예외발생
    host:config.DBINFO.host,      //디비 ip 
    user:config.DBINFO.user,         //디비 계정
    password:config.DBINFO.password,       //디비 계정 패스워드  
    database:config.DBINFO.database,       //디비명     
    port:config.DBINFO.port              //포트번호 
});
